...
Targeted component/API tests cover service visibility alongside a published agreement, no scope mutation, custom-frequency/date payloads, missing custom-frequency validation, preview submissions making no production API calls, and cancellation opening the correct visit-linked form. Production browser checks cover the visible service cards and frequency form; they do not submit real business records.
