...
// Resolver infers optional from schema input; form type requires boolean — cast to match.
...
export default function VendorContestPage() {
  ...
  // Track page view and get partner info
  useEffect(() => {
    async function trackPageView() {
      ...
      try {
        const response = await fetch('/api/tap/track', {
          ...
          body: JSON.stringify({
            partnerId,
            action: 'page_view', // Reusing page_view action to fetch partner info/increment stats
          }),
        })
        ...
        console.error('Failed to track contest view:', error)
      }
    }
    trackPageView()
  }, [partnerId])
    ...
    <div className="relative flex min-h-screen flex-col items-center overflow-hidden p-4 pt-[env(safe-area-inset-top,3rem)] pb-32">
      ...
      <div className="relative z-10 w-full max-w-2xl space-y-8">
        ...
        <div className="text-center">
          ...
          <h1 className="mb-2 text-5xl font-extrabold tracking-tight text-white drop-shadow-lg">
            YOU FOUND SASQUATCH!
          </h1>

          {partnerName && (
            <div className="mb-4 inline-block rounded-full border-2 border-amber-300 bg-amber-500/80 px-4 py-1 text-sm font-bold text-black">
              🎁 Found at: {partnerName}
            </div>
          )}

          <h2 className="mb-4 text-2xl font-bold text-green-400 drop-shadow">
          ...
          <Card className="border-white/20 bg-black/70 p-6">
        ...
        {/* Form */}
        <Card className="border-white/20 bg-black/90 p-6">
