...
|-----------|----------------|
| `20260724120000_add_sds_to_chemical_products.sql` | Adds `chemical_products.sds_url` (link to the manufacturer's official Safety Data Sheet) and `sds_file_url` (public URL of an uploaded PDF copy in the `job-images` bucket under `chemical-sds/`). Techs open a chemical's SDS from `/field/inventory` (the digital "book under the seat"); Charles links/uploads it at `/admin/chemicals`. The spec scraper captures a link to the real sheet only — SDS content is never AI-generated. |
