...
begin
  ...
  if exists (
    ...
    raise exception 'Appointment overlaps an active appointment. Check live availability before booking.'
  ...
  if exists (
    ...
    raise exception 'Appointment overlaps blocked schedule time. Check live availability before booking.'
      using errcode = 'check_violation';
  end if;
...
drop trigger if exists prevent_ai_ops_appointment_overlap_trigger on public.ops_appointments;

create trigger prevent_ai_ops_appointment_overlap_trigger
before insert or update of appointment_date, start_time, end_time, status, booking_channel
on public.ops_appointments
for each row
execute function public.prevent_ai_ops_appointment_overlap();
