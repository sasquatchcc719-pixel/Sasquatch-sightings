-- Subcontracted visits (e.g. Levis Custom Clean / Recovery Village) live on the
-- contractor strip above the schedule, not on our truck calendar. App-level
-- availability already ignores is_subcontracted=true, but this trigger still
-- treated unassigned subcontracted rows as blocking every tech — which made
-- website jobs look "conflicted" and blocked On My Way / booking updates.
...
begin
  ...
  -- Outside crews do not occupy our schedule.
  if coalesce(new.is_subcontracted, false) then
    return new;
  end if;
  ...
  if exists (
    ...
    where existing.id <> coalesce(new.id, '00000000-0000-0000-0000-000000000000'::uuid)
      ...
      and coalesce(existing.is_subcontracted, false) = false
